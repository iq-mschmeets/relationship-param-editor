...
export default class RelationParamEditor extends React.Component {
    ...
    // This method is basically a setter for the models array.
    ...
    reorder(id, from, to) {
        console.log("RelationParamEditor.reorder: %o", arguments);
    }
    oncancel() {}
    onok() {}
    load() {
        ClassService(this.props.classID)
            .then((classObj) => {
                postal.publish({
                    channel: "class-meta-data",
                    topic: "select-class",
...
*/
    render() {
        try {
            ...
            console.log(
                "RelationParamEditor.render: data: %o, state: %o",
                data,
                this.state
            );
            return (
