...
function getDefaultValue( key ){
    let rval = null;
    switch( key ){
        case 'RELATIONSHIP_DISPLAY_EXPANDED' : {rval = false; break;}
        case 'RELATIONSHIP_QUERY_RENDERER'   : {rval = ''; break;}
        default : rval = null;
    }
    return rval;
}
...
export default class ParameterObject {
    ...
    set value( val ) {
        if( !this.hasValue() ){
            ...
            delete this._value;
            if(!this._originalValue ){ this._originalValue = NOT_A_VALID_VALUE; }
        }
